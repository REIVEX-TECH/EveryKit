/**
 * The third breadcrumb segment: which kind of code this path is, if it is one.
 *
 * Null on the kit's own home page and on privacy and terms, which are not
 * tools and get two segments rather than three.
 */

import { getKind } from "@/data/kinds";

export function currentTool(pathname: string): { name: string; href: string } | null {
  const slug = pathname.replace(/^\/+|\/+$/g, "");
  if (slug === "") return null;

  const kind = getKind(slug);
  return kind ? { name: kind.title, href: `/${kind.slug}` } : null;
}
