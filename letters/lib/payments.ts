 ...
 * button hands over the formatted files and the UI shows a launch-week badge.
 * `startCheckout()` is never reached.
 *
 * Paid mode: reading and copying the letter stays free and the formatted files is unlocked
...
const SESSION_KEY = "everykit-letters-unlocked";
...
/** True when the formatted files has been paid for in this session. */
...
export function rememberPaid(): void {
  markPaid();
}
