 ...
 *
 * For this kit the list is the product. The promise is that the song someone
 * opens never leaves the device, and the way that promise is kept honest is that
 * there is no origin here audio could be sent to. The MP3 encoder is bundled and
 * served from this origin, so nothing is fetched from a CDN at runtime.
 *
...
const CONNECT_SOURCES = [
  "'self'",
  // The song, and the MP3 written from it, live in blob URLs so they can be
  // played back and downloaded without a server.
  "blob:",
  "data:",
